...
| **4 — release integrity, docs, signing** ✅ | docs, release CI, signing keys | Honest docs; real minisign signing key committed (key ID `EEDBFDA4BF232D38`; PGP dropped for v0.1.0 — plaintext security reporting); working download-verification chain. Remaining maintainer step before tagging: wire `MINISIGN_SECRET_KEY`/`MINISIGN_PASSWORD` CI secrets |
