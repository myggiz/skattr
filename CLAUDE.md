...
**Phase 0.B (identity & crypto) and Phase 0.C (Arti integration) are complete.** `crates/core/src/identity/` is fully implemented (Ed25519, BIP39, Argon2id + XChaCha20-Poly1305 vault, HKDF). `crates/core/src/transport/tor.rs` + `hs_key.rs` + `listener.rs` wire `arti-client` 0.41 + `tor-hsservice` 0.41 end-to-end. `skattr init` / `skattr restore <seed>` / `skattr daemon` / `--data-dir <path>` work; the daemon bootstraps Tor, publishes a seed-derived v3 onion (HS key at-rest encrypted), and accepts rend requests via `OnionListener`. A cargo-fuzz harness for `Vault::open` is in place. All four crates compile, `cargo clippy -D warnings` and `cargo test` are green, `skattr --help` prints the full subcommand tree. Phase 0 exit criterion (two daemons echo bytes over Tor) is exercised by `crates/tests/src/arti_echo.rs`, `#[ignore]`-gated — run with `cargo test -p skattr-tests --release -- --ignored`. ADR-0004 (passphrase bytes) and ADR-0005 (Arti vs system tor) are committed. Remaining Phase 0 workstreams (0.D Storage layer, 0.E Documentation baseline) still have `todo!()` bodies in `storage/` and downstream modules. Known Phase 0.C deviations tracked as follow-ups: (a) `transport` module was widened from `pub(crate)` to `pub` so the CLI could construct `TorRuntime` directly; Phase 1 should add a `Daemon::run` wrapper and re-narrow. (b) `experimental-api` feature on `arti-client` is used only for `launch_onion_service_with_hsid`; audit on every Arti bump. (c) `is_key_already_exists_error` uses string matching on the error chain; harden before any Arti bump. When filling in further stubs, the bootstrap prompt remains authoritative for file layout, module boundaries, type signatures, and visibility rules — match it exactly.
