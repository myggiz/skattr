...
The Cargo workspace from `docs/skattr-bootstrap-prompt.md` has been scaffolded: all four crates (`core`, `mailbox`, `cli`, `tests`) compile, `cargo clippy -D warnings` and `cargo test` are green, `skattr --help` prints the full subcommand tree. Almost every function body is `todo!()` — the architecture is visible in code, implementations land in **Phase 0 foundations** work (workstreams 0.A–0.E in `docs/skattr-implementation-plan.md`). When scaffolding or filling in stubs, the bootstrap prompt remains authoritative for file layout, module boundaries, type signatures, and visibility rules — match it exactly.
